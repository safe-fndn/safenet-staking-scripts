...
export class SentinelOracle extends EventIndexer<typeof EVENTS, SentinelData> {
	constructor(config: Configuration<SentinelData>) {
		...
		// The indexer's contract identifier is what scopes requests to an
		// oracle. Note that `EventIndexer` already refuses to run against a
		// database that was indexed for a different contract, so the requests
		// stored under this identifier can only ever come from this oracle.
