...
export const logSigningStats = (aggregation: Record<string, SigningStats>, displayFailed: boolean) => {
    ...
    const totalRequests = Object.values(aggregation).filter((sr) => sr.initiatedAt > 0 && sr.committed.length > 0);
    ...
    console.log(`Tracked ${totalRequests.length} signing requests with ${completedRequests.length} signed by threshold (aka completed)`);
    ...
    for (const [validatorId, stats] of validators) {
        ...
        console.log(`Confirmed ${stats.completed}/${completedRequests.length} (${(100 * stats.completed) / completedRequests.length}%)`);
        if (invalidRequests.length > 0) {
            console.log(`Invalid ${stats.invalid}/${invalidRequests.length} (${(100 * stats.invalid) / invalidRequests.length}%)`);
        }
    }
