...
- No behavioural changes to `EventIndexer` — the largest and most delicate piece of existing
  machinery. It gains only a `protected get contract()` exposing the identifier it already computes,
  so that subclasses needing it (here, to scope requests to an oracle) read it off the base class
  rather than recomputing the format and risking drift.
...
**Depends on:** Phase 1
**Files:** `src/indexing/sentinels.ts`, `src/indexing/events.ts`, `src/safenet.ts`,
`src/utils/args.ts`, `tests/harness/scenario.ts`, `.env.sample`, `README.md`
**Estimate:** ~115 LOC, 7 files
